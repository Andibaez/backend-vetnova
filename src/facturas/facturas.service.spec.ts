...
describe('FacturasService', () => {
  ...
  it('crea factura validando relaciones de la misma clínica y calcula total', async () => {
    mockPrisma.propietarios.findUnique.mockResolvedValue({ id_propietario: 7, id_clinica: 1 });
    mockPrisma.mascotas.findUnique.mockResolvedValue({ id_mascota: 5, id_propietario: 7, id_clinica: 1 });
    mockPrisma.productos.count.mockResolvedValue(1);
    mockPrisma.servicios.count.mockResolvedValue(1);
    mockPrisma.$transaction.mockImplementation(async (fn: any) => {
      const tx = {
        facturas: {
          create: jest.fn().mockResolvedValue({ id_factura: 10 }),
          findUnique: jest.fn().mockResolvedValue({ id_factura: 10, total: 120000 }),
        },
      ...
      expect(tx.facturas.create).toHaveBeenCalledWith({
        data: { id_propietario: 7, id_mascota: 5, id_clinica: 1, total: 120000 },
      });
  ...
  it('rechaza factura con mascota de otra clínica', async () => {
    mockPrisma.propietarios.findUnique.mockResolvedValue({ id_propietario: 7, id_clinica: 1 });
    mockPrisma.mascotas.findUnique.mockResolvedValue({ id_mascota: 5, id_propietario: 7, id_clinica: 2 });

    await expect(service.create({ id_propietario: 7, id_mascota: 5 }, adminUser)).rejects.toThrow(
      ForbiddenException,
    );
  });
  ...
  it('rechaza factura sin propietario ni mascota', async () => {
    await expect(
      service.create({ productos: [{ id_producto: 1, cantidad: 1, precio_unitario: 10000 }] }, adminUser),
    ).rejects.toThrow(BadRequestException);
  });

  it('rechaza factura si mascota no pertenece al propietario indicado', async () => {
    mockPrisma.propietarios.findUnique.mockResolvedValue({ id_propietario: 7, id_clinica: 1 });
    mockPrisma.mascotas.findUnique.mockResolvedValue({ id_mascota: 5, id_propietario: 99, id_clinica: 1 });
  ...
  it('actualiza factura accesible de la misma clínica', async () => {
    ...
    expect(mockPrisma.facturas.update).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id_factura: 10 }, data: { total: 90000 } }),
    );
  });
  ...
  it('rechaza actualizar factura de otra clínica', async () => {
    ...
    await expect(service.update(10, { total: 90000 }, adminUser)).rejects.toThrow(ForbiddenException);
  });
  ...
  it('elimina detalles antes de eliminar factura', async () => {
    ...
    expect(mockPrisma.detalle_productos.deleteMany).toHaveBeenCalledWith({ where: { id_factura: 10 } });
    expect(mockPrisma.facturas.delete).toHaveBeenCalledWith({ where: { id_factura: 10 } });
  });
});
