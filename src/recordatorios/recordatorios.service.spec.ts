...
describe('RecordatoriosService', () => {
  ...
  it('admin lista recordatorios por id_clinica de mascota', async () => {
    ...
    expect(mockPrisma.recordatorios.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { mascotas: { id_clinica: 1 } }, take: 10, skip: 0 }),
    );
  });
  ...
  it('crea recordatorio para mascota de la misma clínica', async () => {
    ...
    await service.create({ mensaje: 'Vacuna', fecha_recordatorio: '2026-07-01', id_mascota: 10 }, adminUser);

    expect(mockPrisma.recordatorios.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ mensaje: 'Vacuna', estado: 'pendiente', id_mascota: 10 }),
      }),
    );
  });
  ...
  it('rechaza crear recordatorio para mascota de otra clínica', async () => {
    ...
    await expect(
      service.create({ mensaje: 'Vacuna', fecha_recordatorio: '2026-07-01', id_mascota: 10 }, adminUser),
    ).rejects.toThrow(ForbiddenException);
  });

  it('actualiza y revalida nueva mascota si cambia id_mascota', async () => {
    mockPrisma.recordatorios.findUnique.mockResolvedValue({ id_recordatorio: 1, id_mascota: 10, mascotas: mascota });
    mockPrisma.mascotas.findUnique.mockResolvedValue({ id_mascota: 11, id_propietario: 7, id_clinica: 1 });
    ...
    expect(mockPrisma.mascotas.findUnique).toHaveBeenCalledWith({ where: { id_mascota: 11 } });
  ...
  it('elimina recordatorio accesible', async () => {
    mockPrisma.recordatorios.findUnique.mockResolvedValue({ id_recordatorio: 1, mascotas: mascota });
    mockPrisma.recordatorios.delete.mockResolvedValue({});

    await service.remove(1, adminUser);

    expect(mockPrisma.recordatorios.delete).toHaveBeenCalledWith({ where: { id_recordatorio: 1 } });
  });
});
