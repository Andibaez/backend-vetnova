...
export class UsuariosService {
  ...
  async create(dto: CreateUsuarioDto, user: JwtPayload) {
    ...
    if (roleName === ROLES.SUPER_ADMIN) {
      throw new ForbiddenException(
        'No puedes crear usuarios SuperAdministrador desde una clínica.',
      );
    }
  ...
  async update(id: number, dto: UpdateUsuarioDto, currentUser?: JwtPayload) {
    ...
    if (
      ...
      existing.id_clinica !== currentUser.clinicaId
    ) {
      throw new NotFoundException('Usuario no encontrado.');
    }
    ...
    if (dto.rol) {
      if (dto.rol === ROLES.SUPER_ADMIN) {
        throw new ForbiddenException(
          'No puedes asignar el rol SuperAdministrador desde una clínica.',
        );
      }
  ...
  async remove(id: number, user: JwtPayload) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const vet = await tx.veterinarios.findUnique({
        where: { id_usuario: id },
      });
      if (vet) {
      ...
      await tx.propietarios.updateMany({
        where: { id_usuario: id },
        data: { id_usuario: null },
      });
